        ...
        <ul className="list-group py-1">
          {livros &&
            livros.map((livro, index) => (
              <li className={"list-group-item " + (index === currentIndex ? "active" : "")} onClick={() => setLivroAtivo(livro, index)} key={index}>
                {livro.titulo}
              </li>
            ))}
        </ul>
      ...
      <div className="col-md-6">
        {livroSelecionado ? (
          <div>
            ...
            <Link to={"/livros/" + livroSelecionado.id} className="btn btn-warning">
              Editar
            </Link>
          </div>
        ) : (
